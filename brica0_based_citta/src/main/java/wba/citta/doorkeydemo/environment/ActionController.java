...
public class ActionController {
    ...
    public int[] move(int x, int y, int action) {
        ...
        String mapID = mapData.length() != 0 ? mapData.substring(0, 1): "";
        if( mapID.equals("S") ) {
            /* スタート */
...
//                key = true;
            ...
            /* ジャンプ */
            elem = jumpMove(x, y, action, mapData);
     ...
     * @return int[]     移動先の座標
     */
    private int[] normalMove(int x, int y, int action) {
        /* 要素の取得 */
     ...
     * @return int[]          移動先の座標
     */
    private int[] jumpMove(int x, int y, int action, String mapData) {
