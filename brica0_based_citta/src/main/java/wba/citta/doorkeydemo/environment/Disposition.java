...
public class Disposition {
     ...
     */
    public Disposition(String[][] fileArray) {
     ...
     */
    public static Disposition load(String fileName) throws IOException {
        /* テーブル情報の作成 */
        return new Disposition(makeFileArray(fileName));
    }
     ...
     */
    private static String[][] makeFileArray(String fileName) throws IOException {
        ...
        /* 配列のサイズ設定 */
        String s = mapStringList.get(0);
        StringTokenizer stringTokenizer = new StringTokenizer(s, ",");
        int tokenNum = stringTokenizer.countTokens();

        String[][] fileArray = new String[tokenNum][mapStringList.size()];

        /* 1行分のデータを解析 配列に設定 */
        int lineCnt = 0;
        ListIterator<String> literator = mapStringList.listIterator();
        while(literator.hasNext()) {
            String str = (String)literator.next();
            parseString(fileArray, str, lineCnt);
            lineCnt ++;
        }
        return fileArray;
    }
     ...
     */
    private static void parseString(String[][] fileArray, String mapData, int lineCnt) {
