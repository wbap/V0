...
public class Node implements Serializable{
    ...
    private Map<Integer, Integer> cognitiveDistance;

    /* 直接移動可能な状態のリスト 移動可能なnodeIDのリスト*/
    private List<Integer> forwardNodeIDList;  /* 順方向 */
    private List<Integer> inverseNodeIDList;  /* 逆方向 */

    private int visitCount;              /* この状態への移動回数 */
    /* 状態遷移に関する情報のリスト */
    private Map<Integer, TransitionCounter> transitionCounterTable;
    ...
    public Node(Integer ID, Integer lowerID) {
        ...
        cognitiveDistance = new HashMap<Integer, Integer>(1000); // 失敗
        forwardNodeIDList = new ArrayList<Integer>();
        inverseNodeIDList = new ArrayList<Integer>();
        transitionCounterTable = new HashMap<Integer, TransitionCounter>();
    }
...
//        }
     ...
     */
    public List<Integer> getForwardNodeIDList() {
     ...
     * @return LinkedList 直接移動可能なノードのIDのリスト
     */
    public List<Integer> getInverseNodeIDList() {
    ...
    public List<Integer> getCDKeys() {
        return new ArrayList<Integer>(cognitiveDistance.keySet());
    } 
...
//        }
    ...
    public void setForwardNode(Integer nodeID) {
        ...
        ListIterator<Integer> lIterater = forwardNodeIDList.listIterator(0);
    ...
    public void setInverseNode(Integer nodeID) {
        ...
        ListIterator<Integer> lIterater = inverseNodeIDList.listIterator(0);
        while(lIterater.hasNext()) {
            if(((Integer)lIterater.next()).equals(nodeID)) {
                return;
            }
...
//        return cognitiveDistance.size();
         ...
         */
        int validCDSize = 0;
        for (int distance: cognitiveDistance.values()) {
            if (distance <= maxCDLngth) {
                validCDSize++;
            }
        }
