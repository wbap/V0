...
public class LayeredAgent {
    private static final Logger logger = LoggerFactory.getLogger(LayeredAgent.class);
    /* レイヤーID */
    Integer layerID;
    /* 上位層のエージェント */
    ...
    public LayeredAgent(LayeredAgent layeredAgent, int layerID) {
        ...
        /* 現在の層での状態を管理するVector */
        nodes = new ArrayList<Node>(10000);
        /* StateBufferの最大のサイズはＣＤの最大値＋１ */
        stateBuffer = new StateBuffer(Node.maxCDLngth+1);

        /* パラメータの表示 */
        logger.info("LayerID " + layerID);
        logger.info(" Max CD Length             " + Node.maxCDLngth);
        logger.info(" Shallow Search Length     " + shallowSearchLngth);
        logger.info(" Deep Search Length        " + deepSearchLength);
        logger.info(" Min Search Length         " + minSearchLngth);
        logger.info(" Max Segment Size          " + maxSegmentSize);
        logger.info(" Min Segment Size          " + minSegmentSize);
    }
...
//            upperLayerNodeID = renewUpperIDAndStep(nodeID);
     ...
     *                       のリスト
     */
    private List<Integer> getForwardNodeIDList(Integer nodeID) {
        Node node = getNode(nodeID);
        List<Integer> forwardNodeIDList = node.getForwardNodeIDList();
        // /* リスト内の削除されたノードIDを参照先に置き換え */
        // renewNodeList(forwardNodeIDList);
        /* 自らと同じIDがリスト内にある場合は削除 */
        node.removeSameForwardNodeID();
        /* 置き換えなしで、削除のみ行なう */
        ListIterator<Integer> li = forwardNodeIDList.listIterator();
        while(li.hasNext()) {
     ...
     *                       のリスト
     */
    private List<Integer> getInverseNodeIDList(Integer nodeID) {
        Node node = getNode(nodeID);
        List<Integer> inverseNodeIDList = node.getInverseNodeIDList();
        // ランドマーク削除の処理については保留中
        // /* リスト内の削除されたノードIDを参照先に置き換え */
        ...
        /* 置き換えなしで、削除のみ行なう */
        ListIterator<Integer> li = inverseNodeIDList.listIterator();
        while(li.hasNext()) {
    ...
    private void renewNodeIDList(List<Integer> nodeIDList) {
        for (ListIterator<Integer> li = nodeIDList.listIterator(); li.hasNext();) {
            Integer nodeID = li.next();
    ...
    public void learn(Integer id_S) {
        ...
        if(learningFlag) {
            ...
            if( (id_S != null) && ( (stateBuffer.size() == 0) ||
                    (!id_S.equals(stateBuffer.get(stateBuffer.size() - 1))) ) ) {
                /* この層の学習 */
//                System.out.println();
...
//                }
    ...
    private boolean isReachSearchDeep(List<Integer> list, Integer target) {
        Set<Integer> checkTable = new HashSet<Integer>();
        for (int i = 0; i < deepSearchLength; i++) {
            List<Integer> nextList = getNextList(checkTable, list);
     ...
     * 状態があるか探索します。
     */
    private boolean searchReachableState(List<Integer> list, Integer target) {
    ...
    private List<Integer> getNextList(Set<Integer> checkTable, List<Integer> list) {
        List<Integer> nextList = new ArrayList<Integer>();
        for (final int id: list) {
            Node node = getNode(id);
            List<Integer> forwardNodeIDList = node.getForwardNodeIDList();
            for (int forwardNodeID: forwardNodeIDList) {
                if (!checkTable.contains(forwardNodeID)) {
                    checkTable.add(forwardNodeID);
...
//            }
    ...
    private Object[] getNextNodeInfoFromStateList(StateList stateList,
        ...
        ListIterator<Integer> stateIterator
     ...
     *                       StateListの配列
     */
    private StateList[] getChildListArray(StateList[] stateListArray, Set<Integer> checkTable) {
        ...
        int n = 0;
        for(int i = 0; i < stateListArray.length; i++) {
            ListIterator<Integer> il = stateListArray[i].listIterator();
            while( il.hasNext() ) {
     ...
     */
    private StateList getChildList(Integer parentNodeID, StateList stateList, Set<Integer> checkTable) {
        /* 順方向に移動可能な状態を取得 */
        List<Integer> forwardNodeIDList = getForwardNodeIDList(parentNodeID);
        /* すでに使用済みのノード以外を使用 */
        List<Integer> checkedforwardNodeIDList = new ArrayList<Integer>();
        for (Integer nodeID: forwardNodeIDList) {
    ...
    private Integer getNovelNodeID(Integer nodeID) {
        ...
        Node node = getNode(nodeID);
        List<Integer> ll = node.getCDKeys();
        /* 各ノードの移動回数をチェック */
        for (Integer cdNodeID: ll) {
    ...
    private void learning(Integer nodeID) {
         ...
         */
        int distance = 1;
        ListIterator<Integer> stateBufferIterater
        ...
        while(stateBufferIterater.hasPrevious()) {
             ...
             */
            List<Integer> forwardNodeIDList = getForwardNodeIDList(nodeID);
            for (Integer forwardNodeID: forwardNodeIDList) {
...
//                    }
    ...
    List<Integer> noLandmarkNodeIDBuffer = new ArrayList<Integer>();
    ...
    private void setLandmarkToBuffer(Integer landmarkID, int step) {
        // リストの全状態にランドマークを設定
        ListIterator<Integer> li = noLandmarkNodeIDBuffer.listIterator();
    ...
    */
    ...
    private Object[] getNearestLandmark(Integer nodeID) {
        ...
        List<Integer>[] searchNodeIDList = new ArrayList[maxSegmentSize];
     ...
     */
    private List<Integer> getMoveableNodeList(List<Integer> nodeList,
            boolean direction, Set<Integer> checkTable){
        ...
        for (Integer nodeID: nodeList) {
            ...
            while(listIterator.hasNext()) {
                ...
                if(!checkTable.contains(nextNodeID)) {
                    nextNodeList.add(nextNodeID);
                    checkTable.add(nextNodeID);
                }
            }
        }
...
// パスの学習も行なわないようにする
    ...
    public void load(ObjectInputStream oInputStream) throws IOException,
        ...
        nodes = (List<Node>)oInputStream.readObject();
