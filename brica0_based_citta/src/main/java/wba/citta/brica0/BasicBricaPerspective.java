...
public abstract class BasicBricaPerspective extends Module implements ISharedMemory, ILatch, IGoalStack {
    final Logger logger = LoggerFactory.getLogger(BasicBricaPerspective.class);
    final int size;
    ...
    public State getState(boolean[] useNode) {
        ...
        final Goal state = getState();
        State retval = new State(c);
        for (int i = 0, j = 0; i < l; i++) {
            if (useNode[i]) {
                retval.set(j, state.get(i));
                j++;
            }
        }
        return retval;
    ...
    public BasicBricaPerspective(int size,
