...
public class VectorTest {
    ...
    public void test3() {
        ...
        float res = Vector.normalize(v);
        assertEquals("normalize(MAX_VALUE/2)", (float) (Short.MAX_VALUE / 2)
                / (float) Short.MAX_VALUE, res, 1e-6f);
    }
    ...
    public void test4() {
        ...
        float[] res = Vector.normalize(v);
        assertEquals(
                "normalize(array): input and output array length mismatch",
                v.length, res.length);
        assertTrue("normalize(array) wrong result", res[0] == 0.0f
                && res[1] == 1.0f && res[2] - 0.5f < 1e-7f);
    }
